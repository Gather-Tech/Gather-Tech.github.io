...
TensorFlowを使ってマリオカートの操作を学習させて自動運転させる話。トレーニングデータを取得する部分とN64エミュレーターを使って操作する部分が大変そう。

### [python で 分割ダウンロード : Qiita](http://qiita.com/code_monkey/items/25e96431c3533d9cd35a)

Pythonで大きいファイルをダウンロードするときに分割して行う方法。リクエストヘッダーにRangeヘッダーを付与して分割する。threadingでスレッド分割すればさらに速くできる。
