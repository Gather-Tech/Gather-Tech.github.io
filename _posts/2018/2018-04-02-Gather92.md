...
### [Acer、世界初のChrome OSを搭載した教育向け9.7型タブレット - PC Watch](https://pc.watch.impress.co.jp/docs/news/1113686.html)

Chrome OSタブレットが発表された。ディスプレイにワコムのEMR方式デジタイザが内蔵されている。4月から米国で販売予定とのこと。
...
Kotlinを使った方がよい理由について書かれている。Javaと100%互換性がある、Null Safety、Lambdaのわかりやすさなど。

### [Google Cloud Platform Japan 公式ブログ: ラーメン二郎とブランド品で AutoML Vision の認識性能を試す](https://cloudplatform-jp.googleblog.com/2018/03/automl-vision-in-action-from-ramen-to-branded-goods.html)

GoogleのAutoML Visionの活用例の紹介。ラーメン二郎の識別、メルカリのブランド品分類の事例が取り上げられている。
...
### [GoogleCloudPlatform/skaffold: GitHub](https://github.com/GoogleCloudPlatform/skaffold)

Kubernetesアプリの継続的な開発をサポートするツール。コードの書き換えを行うと自動的にビルドとデプロイが行われる。
