...
### [自分だけのAmazon Alexa スキルを簡単に作成できるツール「Alexa Skill Blueprints」が日本でも使えるようになりました : Alexa Blogs](https://developer.amazon.com/ja/blogs/alexa/post/9dcb1b5b-cfb5-45e7-8013-c56d6772332d/alexa-skill-blueprints-is-available-in-japan)

AlexaスキルをGUIで簡単に作れるAlexa Skill Blueprintsが日本でも使えるように。
...
### [mixi における不正ログイン防止についての取り組み | mixi運営者ブログ : mixi公式サイト](https://sns.mixi.co.jp/blog/20190318-1.html)

mixiでの不正ログイン対策について。reCAPTCHA v2導入とそれにより不正ログインがどのくらい減ったか、reCAPTCHA v3の導入について書かれている。v3は画像を選択させる画面が出なく不正アクセスの可能性を表すスコアを返す。
...
### [How to 速度改善 ーWebパフォーマンスについて知っておきたいこと7選ー : Qiita](https://qiita.com/masanarih0ri/items/1c6b856da0bd04bed995)

Webパフォーマンスを改善するために知っておきたい知識について。ブラウザレンダリングの仕組み、Response/Animation/Idle/Loadについて、パフォーマンスの計測方法についてなど。

### [FCM が端末に送るメッセージの生存期間: Google Developers Japan](https://developers-jp.googleblog.com/2019/03/fcm.html)

Firebase Cloud Messagingのメッセージが配信される仕組みについて。メッセージ受け入れの仕組み、Android/iOS/Webでの端末へのメッセージ配信の仕組みが書かれている。

### [Go 2018 Survey Results : The Go Blog](https://blog.golang.org/survey2018-results)

Go 2018 Surveyの結果。Goの利用用途で多いのはAPI/RPCサービスとCLIツール、エディタはVS CodeとGoLandが多いなど。
...
### [スマホブラウザ「Smooz」のマネタイズで得た知見 : Speaker Deck](https://speakerdeck.com/maxkato/sumahoburauza-smooz-falsemanetaizudede-tazhi-jian)

スマホアプリのマネタイズについて。売り上げ推移や各施策によって何がどれくらい上がったのかも書かれていて分かりやすい。
...
### [telepresence で始める k8s 時代のローカル開発 : Speaker Deck](https://speakerdeck.com/shiro16/telepresence-deshi-meru-k8s-shi-dai-falserokarukai-fa?slide=7)

telepresenceについて。telepresenceとは何か、使い方、ハマりポイントなどが書かれている。リモートのk8sクラスタに接続しながらローカルで実行できる。
...
### [Nick-Gottschlich/Social-Amnesia : GitHub](https://github.com/Nick-Gottschlich/Social-Amnesia)

redditとTwitterの過去の投稿などを削除できるツール。

### [Handsontable | JavaScript Data Grid Component For Web Apps](https://handsontable.com/)

JavaScriptのData Gridライブラリ。React、Vueのコンポーネントも用意されている。商用利用だと有料。

### [Tonejs/Tone.js: GitHub](https://github.com/Tonejs/Tone.js)

Web Audio APIを使ったブラウザ上でインタラクティブな音楽を作れるJavaScriptライブラリ。MIDIフィルをTone.jsが解釈できるJSONフォーマットに変換できたりもする。
...
### [パーソナル・コーチング【mento -メント】](https://www.mento.jp/)

パーソナルコーチングサービス。お試し5000円/1時間、継続セッション15,000円/月～。
